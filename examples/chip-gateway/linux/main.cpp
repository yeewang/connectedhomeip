                                        ...
                                        uint16_t manufacturerCode, uint8_t type, uint8_t size, uint8_t * value)
{
    ...
    if (*value)
    {
        GatewayMgr().InitiateAction(GatewayManager::ON_ACTION);
    }
    else
    {
        GatewayMgr().InitiateAction(GatewayManager::OFF_ACTION);
    }
}
...
exit:
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to run Linux Gateway App: " << ErrorStr(err) << std::endl;
